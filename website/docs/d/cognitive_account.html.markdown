...
* `custom_question_answering_search_service_id` - If `kind` is `TextAnalytics` the ID of the search service.

* `custom_question_answering_search_service_key` - If `kind` is `TextAnalytics` the key of the search service.

* `custom_subdomain_name` - The subdomain name used for token-based authentication.
...
* `metrics_advisor_aad_client_id` - If kind is `MetricsAdvisor` the Azure AD Client ID (Application ID).

* `metrics_advisor_aad_tenant_id` - If kind is `MetricsAdvisor` the Azure AD Tenant ID.

* `metrics_advisor_super_user_name` - If kind is `MetricsAdvisor` the super user of Metrics Advisor.

* `metrics_advisor_website_name` - If kind is `MetricsAdvisor` the website name of Metrics Advisor.
...
* `project_management_enabled` -  If `kind` is `AIServices` whether project management is enabled.
...
* `storage_account_id` - Full resource id of a Microsoft.Storage resource.
